...
protected:
	...
	DECLARE_MESSAGE_MAP()
public:
	VOID CCcRemoteDlg::ChangeRemark(CString);
...
#define COLUMN_ONLINE_COUNT 6			// 在线列表的个数
#define COLUMN_MESSAGE_COUNT 3			// 消息列表的个数
	...
	// 添加信息到列表							
	void	AddList(CString strIP, CString strAddr, CString strPCName, CString strOS,  CString strPing, ClientContext*pContext);
...
public:
	...
	afx_msg void OnOnlineRemark();
};
VOID CCcRemoteDlg::ChangeRemark(CString sztmpRemark)
{
	CString strIP;
	int iSelect = m_CList_Online.GetSelectionMark();
	m_CList_Online.SetItemText(iSelect, ONLINELIST_REMARK, sztmpRemark);
	// 获得选定行的索引
