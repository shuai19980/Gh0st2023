...
BEGIN_MESSAGE_MAP(CCcRemoteDlg, CDialogEx)
	...
	ON_WM_ERASEBKGND()
	ON_WM_CTLCOLOR()
	ON_COMMAND(ID_ONLINE_REMARK, &CCcRemoteDlg::OnOnlineRemark)
END_MESSAGE_MAP()
...
int CCcRemoteDlg::InitMyMenu()
{
	...
	CBitmap bmp;
	CBrush m_BKBrush;
	bmp.LoadBitmap(IDB_BITMAP_IMGBK);
...
void CCcRemoteDlg::AddList(CString strIP, CString strAddr, CString strPCName, CString strOS,  CString strPing, ClientContext*pContext)
{
	...
	m_CList_Online.SetItemText(0, ONLINELIST_REMARK, "");
	...
	try
	{
		...
		case VER_PLATFORM_WIN32_NT:
			...
			if (LoginInfo->OsVerInfoEx.dwMajorVersion == 6 && LoginInfo->OsVerInfoEx.dwMinorVersion == 1)
				pszOS = "Windows7";
			if (LoginInfo->OsVerInfoEx.dwMajorVersion > 6)
				pszOS = "Windows10";
		}
		strOS.Format
		(
		...
		if (((CCcRemoteApp *)AfxGetApp())->m_bIsQQwryExist)
		{
			...
			AddList(strIP, strAddr, strPCName, strOS,  strPing, pContext);
	}
	catch (...) {}
...
void CCcRemoteDlg::OnOnlineRemark()
{
	// TODO: 在此添加命令处理程序代码
	Remark remarkDlg;
	remarkDlg.DoModal();
