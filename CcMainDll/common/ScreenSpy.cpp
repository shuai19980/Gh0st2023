...
#include <stdio.h>
#endif
CSsy::CSsy(int biBitCount, bool bIsGray, UINT nMaxFrameRate)
{
	switch (biBitCount)
	{
	case 1:
...
LPVOID CSsy::getNextScreen(LPDWORD lpdwBytes)
{
...
bool CSsy::ScanChangedRect(int nStartLine)
{
	bool	bRet = false;
	LPDWORD p1, p2;
...
void CSsy::setAlgorithm(UINT nAlgorithm)
{
...
// 创建位图结构
LPBITMAPINFO CSsy::ConstructBI(int biBitCount, int biWidth, int biHeight)
{
/*
...
UINT CSsy::getFirstImageSize()
{
	return m_lpbmi_full->bmiHeader.biSizeImage;
}


void CSsy::setCaptureLayer(bool bIsCaptureLayer)
{
	DWORD dwRop = SRCCOPY;
	if (bIsCaptureLayer)
		dwRop |= CAPTUREBLT;
...
bool CSsy::SelectInputWinStation()
{
...
void CSsy::ScanScreen( HDC hdcDest, HDC hdcSrc, int nWidth, int nHeight)
{
	UINT	nJumpLine = 50;
...
// 差异比较算法块的函数
int CSsy::Compare( LPBYTE lpSource, LPBYTE lpDest, LPBYTE lpBuffer, DWORD dwSize )
{
